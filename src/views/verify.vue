...
async function sendVerify() {
    try {
        ...
        const res = await axios.get(
            `http://spotify_clone_api.test/api/email/verify?id=${authStore.user.id}`,
        );
        console.log(res);
...
<template>
    ...
    <div
        class="container mx-auto flex h-screen flex-col items-center justify-center gap-6 text-white"
    >
        ...
        <RouterLink
            to="/login"
            class="text-3xl text-[#BC4D15] underline hover:opacity-75"
        >
            Xác nhận
        </RouterLink>
    </div>

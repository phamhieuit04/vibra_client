<template>
  <div class="min-h-screen w-full bg-[#0A0A0A] relative flex items-center justify-center p-6 font-sans overflow-hidden">
    ...
    <!-- Main Content -->
    <div class="relative max-w-md w-full bg-gradient-to-br from-[#1DB954]/80 to-[#168740]/80 rounded-3xl overflow-hidden text-[#FFFFFF] shadow-2xl z-10 transform transition-all duration-500 hover:shadow-3xl hover:-translate-y-2">
      ...
      <div class="relative p-10 text-center">
        ...
        <div class="relative mx-auto w-56 h-56 rounded-full bg-[#121212] flex items-center justify-center border-4 border-[#1DB954]/40 shadow-[0_0_20px_rgba(29,185,84,0.6),_0_0_40px_rgba(29,185,84,0.3)] transition-all duration-500 hover:shadow-[0_0_30px_rgba(29,185,84,0.8),_0_0_60px_rgba(29,185,84,0.5)]">
          <!-- Neon Glow -->
          <div class="absolute inset-0 rounded-full bg-gradient-to-r from-[#1DB954]/50 to-[#168740]/50 animate-neon-glow filter blur-sm"></div>
          <!-- Bloom Effect -->
          <div class="absolute inset-0 rounded-full bg-gradient-to-r from-[#1DB954]/40 to-[#168740]/40 animate-bloom filter blur-md"></div>
          <!-- Lens Flare Effect -->
        ...
        <div class="relative flex items-center justify-center mt-6 space-x-2">
          <div v-for="i in 9" :key="i" class="w-1 bg-gradient-to-b from-[#1DB954]/90 to-[#168740]/90 rounded-full animate-wave" :style="{ animationDelay: `${i * 0.08}s`, height: `${10 + i * 4}px` }"></div>
        </div>
        <!-- Snowfall Effect -->
...
.particle-1 { width: 3px; height: 3px; top: 15%; left: 25%; animation: orbit 5s infinite ease-in-out; }
.particle-2 { width: 4px; height: 4px; top: 35%; left: 65%; animation: orbit 4s infinite ease-in-out reverse; }
.particle-3 { width: 5px; height: 5px; top: 55%; left: 20%; animation: orbit 6s infinite ease-in-out; }
.particle-4 { width: 4px; height: 4px; top: 45%; left: 75%; animation: orbit 5s infinite ease-in-out reverse; }
.particle-5 { width: 6px; height: 6px; top: 25%; left: 40%; animation: orbit 7s infinite ease-in-out; }
.particle-6 { width: 5px; height: 5px; top: 60%; left: 60%; animation: orbit 6s infinite ease-in-out reverse; }

@keyframes orbit {
  0% { transform: translate(0, 0) rotate(0deg) scale(1); opacity: 0.8; }
  25% { transform: translate(30px, -30px) rotate(90deg) scale(1.3); opacity: 1; }
  50% { transform: translate(0, -60px) rotate(180deg) scale(1); opacity: 0.8; }
  75% { transform: translate(-30px, -30px) rotate(270deg) scale(1.3); opacity: 1; }
...
@keyframes neon-glow {
  0% { box-shadow: 0 0 10px rgba(29, 185, 84, 0.7), 0 0 20px rgba(29, 185, 84, 0.4); }
  100% { box-shadow: 0 0 20px rgba(29, 185, 84, 0.9), 0 0 40px rgba(29, 185, 84, 0.6); }
}
...
@keyframes glow {
  0%, 100% { box-shadow: 0 0 25px rgba(29, 185, 84, 0.5), 0 0 50px rgba(29, 185, 84, 0.3); }
  50% { box-shadow: 0 0 40px rgba(29, 185, 84, 0.7), 0 0 80px rgba(29, 185, 84, 0.4); }
}
...
.ray-1 { top: 0; left: 50%; transform: rotate(45deg); animation: ray-pulse 3s infinite ease-in-out; }
.ray-2 { top: 0; left: 50%; transform: rotate(-45deg); animation: ray-pulse 3.5s infinite ease-in-out; }
.ray-3 { top: 0; left: 50%; transform: rotate(135deg); animation: ray-pulse 4s infinite ease-in-out; }
.ray-4 { top: 0; left: 50%; transform: rotate(-135deg); animation: ray-pulse 4.5s infinite ease-in-out; }
...
@keyframes fall {
  0% { transform: translateY(-100%) rotate(0deg); opacity: 0.3; }
  50% { opacity: 0.5; }

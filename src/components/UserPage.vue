...
async function getBills() {
    try {
        const res = await axios.get(
            `http://spotify_clone_api.test/api/profile/payment-history`,
            {
                headers: {
...
async function createAlbum() {
    try {
        const res = await axios.get(
            `http://spotify_clone_api.test/api/profile/create-album`,
            {
                headers: {
                    Authorization: 'Bearer ' + authStore.user.token,
                },
...
<template>
    <div
        class="h-[76%] w-full space-y-10 rounded-[24px] bg-[#1D1512] text-white"
    >
        <div
            class="flex h-52 items-center space-x-7 bg-gradient-to-b from-[#1D1512]/20 to-[#312825]/100 px-10 py-6"
        >
            <div
                class="aspect-square h-[160px] w-[160px] rounded-full bg-gray-600"
            >
                <img
                    class="aspect-square h-[160px] w-[160px] rounded-full object-cover"
        ...
        <div
            class="scrollbar-style relative h-[calc(100vh-450px)] overflow-y-auto px-10"
        >
            <button
                class="mb-5 rounded-full p-2 text-3xl text-zinc-400 hover:bg-zinc-600 hover:text-white"
            ...
            <span
                v-if="openMenu"
                class="absolute left-10 top-14 z-20 w-[200px] bg-[#282828] p-1"
            >
                <ul class="text-[14px] font-semibold text-gray-200">
                    <li
                        class="cursor-pointer px-3 py-2 hover:bg-[#3E3D3D]"
            ...
            <div class="mb-3 mt-8" v-if="mySongList.length > 0">
                ...
                <div class="scrollbar-style overflow-x-auto">
                    <div class="flex w-max space-x-7 px-1 py-2">
                        ...
                        >
                            <div
                                class="mb-5 h-32 w-32 rounded-full bg-zinc-700"
                            >
                                <img
            ...
            <div class="mt-8" v-if="followArtistList.length > 0">
                ...
                <div class="scrollbar-style overflow-x-auto">
                    <div class="flex w-max space-x-7 px-1 py-2">
                        <div
                            v-for="item in followArtistList"
                            :key="item.id"
            ...
            <div v-if="bills.length > 0">
                ...
                <div
                    class="scrollbar-style max-h-[30rem] overflow-y-auto rounded-lg border border-gray-700"
                >
                    <table class="min-w-full bg-[#1a1a1a]">
                        <thead class="sticky top-0 bg-[#2a2a2a]">
                            <tr>
                                <th
                                    class="px-4 py-3 text-left text-sm font-semibold text-[#FFE5D6]"
                                >
                                    Mã giao dịch
                                </th>
                                <th

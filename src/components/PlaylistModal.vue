...
const saveProfile = async () => {
    ...
    try {
        const res = await api.post(`/library/update-playlist/${playlistEditData.value.id}`,
            formData, {
            headers: {
...
<template>
    <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-60">
        <div class="relative w-[550px] rounded-lg bg-[#1D1512] p-6 text-white">
            <button class="absolute right-4 top-4 text-white hover:text-red-500" @click="openEditPlaylist = false">
                ✕
            </button>
            ...
            <div class="mb-3 flex gap-1">
                ...
                <div class="ml-2 flex-1">
                    <input v-model="playlistName" class="mb-3 w-full rounded-lg bg-[#25211F] px-4 py-3 text-white"

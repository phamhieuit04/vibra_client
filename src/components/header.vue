...
async function getAllCategories() {
    try {
        const res = await axios.get(
            'http://spotify_clone_api.test/api/category/index',
            {
                headers: {
...
async function logout() {
    try {
        const res = await axios.get(
            'http://spotify_clone_api.test/api/logout',
            {
                headers: {
...
<template>
    <div
        class="fixed right-0 z-20 flex h-[64px] w-[100%] items-center justify-between bg-[#BC4D15]"
        :style="{ backgroundColor: useView.currentColor }"
    >
        <div class="relative ml-8 flex items-center gap-3">
            <img
                class="size-20 cursor-pointer text-[64px] text-white transition duration-200 hover:text-black"
                :src="LogoText"
                @click="
                    useView.setComponent('HomePage');
            ...
            <div
                class="relative ml-0 h-8 w-8 cursor-pointer rounded-full bg-black shadow-2xl hover:scale-105"
            >
                <div
            ...
            >
                <div
                    class="mx-2 h-6 w-6 cursor-pointer rounded-full hover:scale-110"
            ...
            <div class="flex items-center gap-3">
                ...
                >
                    <Icon
                        ...
                        class="size-8 cursor-pointer transition duration-200"
                        :class="
                            useView.currentComponent === 'HomePage'
                                ? 'text-[#FFE5D6]'
                                : 'text-[#FFE5D6]/30'
                        "
                    />
                ...
                >
                    <div
                        class="flex w-full items-center gap-2 border-r-2 border-[#7c7c7c]"
                    >
                        <Icon
                            icon="material-symbols:search-rounded"
                            class="size-8 cursor-pointer transition duration-200"
                            :class="
                                useView.currentComponent === 'SearchPage'
                                    ? 'text-[#FFE5D6]'
                                    : 'text-[#FFE5D6]/30'
                            "
                        />
                        <input
                            v-model="searchValue"
                            @input="useActivity.changeSearchKey(searchValue)"
                            type="text"
                    ...
                    <Icon
                        icon="fluent:collections-empty-16-filled"
                        class="cursor-pointer text-3xl transition duration-200 hover:scale-110"
                        @click="
                            useView.setComponent('CategoriesPage');
                            useView.selectItem(this);
                        "
                        :class="
                            useView.currentComponent === 'CategoriesPage'
                                ? 'text-[#FFE5D6]'
                                : 'text-[#FFE5D6]/30'
                        "
                    />
        ...
        <div class="flex items-center gap-3">
            <div
                class="flex h-[44px] items-center justify-center rounded-full font-bold text-white"
            >
                <button
                    @click="
                ...
                >
                    <div
                        class="flex h-[43px] items-center rounded-full bg-gray-500 p-[2px]"
                        width="44"
                    >
                        <img
